...
let print_parser p = 
  "[Parser]: Current: ->" ^ (Token.print_token p.current_tok)
  ^ "<- Peek: ->" ^  (Token.print_token p.peek_tok) ^ "<-" 
...
let expect_token_equal expectation actual = 
  if expectation = actual then true else (print_endline (Printf.sprintf "Expected: %s Actual: %s" (Token.print_token (Some expectation)) (Token.print_token (Some actual))); false)
...
let rec parse_json_array parser = 
  ...
  let rec parse_array_aux parser objects = 
    match parser.current_tok with
      ...
      | Some t when (is_valid_object_start (Some t)) -> (* Should be the beginning of a new object*)
          let parser, obj = parse_json_object parser in
          let parser = expect_peek (fun t -> print_endline "]OR,";(t = Token.Comma || t = Token.RSquareBracket)) parser in
          parse_array_aux (advance parser) (obj :: objects)
...
and parse_named_object parser =
  ...
  let key = match key with
    ...
    | Some t -> t
  in
  let parser = advance parser in (* Process the colon*)
  let parser = advance parser in
  match parser.current_tok with
    | None -> failwith "Invalid Json: Ends in :"
    | Some t -> match t with
      | Token.Word s -> parser, Ast.Named{key = key; value = Ast.Unnamed(Token.Word(s))}
      | Token.LParen -> parse_json_object parser
      | Token.LSquareBracket -> parse_json_array parser
      | _ -> failwith "Invalid Json: either an array, a string or a new object must follow a :"
...
and parse_json_object parser =
  ...
  match parser.current_tok with
    ...
    | Some t -> match t with
      | Token.LParen -> let _, nested = parse_json_object (advance parser) in parser, Ast.Nested( nested )
      | Word _ when parser.peek_tok = Some Token.Collon ->
        let parser, objects = parse_named_object parser in
        let parser = advance parser in parser, objects
      | Word w ->
          let parser = advance parser in parser, Ast.Unnamed(Token.Word w)
...
let parse parser =
  ...
  let rec parse_aux parser objects = 
    match parser.current_tok with
      ...
      | Some _ -> (* Should be the beginning of a new object*)
          let parser, obj = parse_json_object parser in
