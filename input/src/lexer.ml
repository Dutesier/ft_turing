...
let advance = function
  | {input; position = p; ch} when ch <> None -> begin
    {
      input;
      position = p + 1;
...
let skip_whitespace lexer =
  let lex, _ = seek lexer (fun ch -> begin
    match ch with 
      | None -> false
...
let read_word lexer = 
  let lexer = advance lexer in (*Advance from the first quote*)
  ...
  seek lexer (fun ch -> 
    match ch with
      ...
      | None -> false)
  in (* Now lexer is poiting at end quote*)
...
let next_token (lexer : t) = 
  ...
  | Some ch ->
    let lexer, token =
      match ch with
      | '"' ->  let lex, tok = read_word lexer in lex, lookup_words tok
